...
int playerX, playerY, finalX[7], finalY[7], destCount = 0;
...
void rules() {
        ...
        case LEFT:
            if(map[playerY][playerX-1] != '#') {
                if(map[playerY][playerX-1] == 'B' && map[playerY][playerX-2] != '#' && map[playerY][playerX-2] != 'B') {
                    if(map[playerY][playerX-2] == 'x') destCount++;
                    map[playerY][playerX] = ' ';
                    map[playerY][playerX-1] = '@';
                    map[playerY][playerX-2] = 'B';
                    playerX--;
                } else if(map[playerY][playerX-1] == ' ' && map[playerY][playerX-1] != 'B') {
                    map[playerY][playerX] = ' ';
                    map[playerY][playerX-1] = '@';
                    playerX--;
                }
        ...
        case UP:
            if(map[playerY-1][playerX] != '#') {
                if(map[playerY-1][playerX] == 'B' && map[playerY-2][playerX] != '#' && map[playerY-2][playerX] != 'B') {
                    ...
                    playerY--;
                } else if(map[playerY-1][playerX] == ' ' && map[playerY-1][playerX] != 'B') {
                    map[playerY][playerX] = ' ';
                    map[playerY-1][playerX] = '@';
                    playerY--;
                }
    ...
    for(int i = 0; i < 7; i++) {
         if(map[finalY[i]][finalX[i]] != 'x' && map[finalY[i]][finalX[i]] != 'B') {
...
int main() {
    ...
    printMap();
    mvprintw(WIN_HEIGHT/2,WIN_WIDTH/3,"That's All!");
    refresh();
    getch();
    endwin();
    return 0;
